...
class InsSegTester(TesterBase):
    ...
    def RUNNING_associate_instances(self, pred, segment, name):
        segment = segment.cpu().numpy()
        void_mask = np.in1d(segment, (-1))
    ...
    def associate_instances(self, pred, segment, instance):
        ...
        void_mask = np.in1d(segment, segment_ignore_index)
        ...
        gt_instances = dict()
        for i in range(self.cfg.data.num_classes):
            if i not in segment_ignore_index:
        ...
        for i in range(len(instance_ids)):
            if instance_ids[i] == instance_ignore_index:
                continue
            if segment_ids[i] in segment_ignore_index:
                continue
            gt_inst = dict()
        ...
        instance_id = 0
        for i in range(len(pred["pred_classes"])):
            if pred["pred_classes"][i] in segment_ignore_index:
                continue
            pred_inst = dict()
            pred_inst["uuid"] = uuid4()
            pred_inst["instance_id"] = instance_id
            ...
            if pred_inst["vert_count"] < min_region_sizes:
                    ...
                    for scene in scenes:
                        ...
                        for pred in pred_instances:
                            ...
                            if not found_gt:
                                ...
                                for gt in pred["matched_gt"]:
                                    if gt["segment_id"] in segment_ignore_index:
    ...
    def test(self):
        ...
        for i, input_dict in enumerate(self.test_loader):
            ...
            # print('output dict = ', output_dict["pred_masks"].shape)
            ...
            # scenes.append(dict(pred=pred_instance))
            ...
            # print(pred_instance, i)
            # print('input_dict coord = ', input_dict['coord'].shape)
            # print('input_dict origin_coord = ', input_dict['origin_coord'].shape)
            # print('output_dict pred_masks = ', output_dict['pred_masks'].shape)
            # print('output_dict pred_classes = ', output_dict['pred_classes'].shape)
            # print('output_dict pred_scores = ', output_dict['pred_scores'].shape)
            

            # pred_save_path = os.path.join(save_path, "orig_coord_pred.txt")
            # np.savetxt(pred_save_path, input_dict["origin_coord"].cpu().numpy())

            pred_save_path = os.path.join(save_path, "instance_pred.npy")
            np.save(pred_save_path, output_dict["pred_masks"].cpu().numpy())
