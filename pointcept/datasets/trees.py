...
class TreesDataset(Dataset):
    def __init__(
        ...
        transform=None,
        test_mode=False,
        instance_mode=False, # Added for instance segmentation
        test_cfg=None,
        cache=False,
        loop=1,
        ignore_index=-1,
    ):
        super(TreesDataset, self).__init__()
        ...
        self.instance_mode = instance_mode # Added for instance segmentation
    ...
    def prepare_test_data(self, idx):
        # load data
        data_dict = self.get_data(idx)
        # data_dict = self.transform(data_dict) # Added for instance segmentation
        # ''' # Commented for instance segmentation
    ...
    def prepare_test_instance_data(self, idx):
        # load data
        data_dict = self.get_data(idx)
        data_dict = self.transform(data_dict) # Added for instance segmentation
        ''' # Commented for instance segmentation
    ...
    def __getitem__(self, idx):
        if self.test_mode:
            if self.instance_mode:
                return self.prepare_test_instance_data(idx)
            else:
