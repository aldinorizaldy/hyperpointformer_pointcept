...
class CrossAttentionOffset(nn.Module):
    """
    Cross-Attention for multimodal fusion with Pointcept-style batching.
    Processes [N, C] features grouped by 'offset' (per sample).
    """
    ...
    def forward(self, feat_q, feat_kv, offset):
        """
        feat_q: [N, C] - query modality (e.g., geometry)
        feat_kv: [N, C] - key/value modality (e.g., spectral)
        offset: [B] - cumulative count of points per batch (Pointcept format)
        """
        N, C = feat_q.shape
        H, D = self.num_heads, self.head_dim

        q = self.q_proj(self.ln_q(feat_q))
        k = self.k_proj(self.ln_kv(feat_kv))
        v = self.v_proj(self.ln_kv(feat_kv))
        ...
        start = 0
        for end in offset:  # iterate per batch
            q_b = q[start:end]  # [Ni, H, D]
            k_b = k[start:end]
            v_b = v[start:end]

            # Compute attention within this batch
            attn = torch.einsum("ihd,jhd->hij", q_b, k_b) * self.scale
